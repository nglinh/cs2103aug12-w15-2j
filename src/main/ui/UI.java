...
public abstract class UI {
	...
	private static final DateTimeFormatter LINE_DATE_FORMATTER = DateTimeFormat
			.forPattern(LINE_DATE_FORMAT);
	private static final DateTimeFormatter LINE_DATE_LONGER_FORMATTER = DateTimeFormat
			.forPattern(LINE_DATE_LONGER_FORMAT);
	...
	protected static NattyParserWrapper nattyParser = NattyParserWrapper
			.getInstance();

	public static final String[] fontsPreferred = { "Segoe UI",
			"Lucida Grande", "Lucida Sans Unicode", "Lucida Sans",
			"Bitstream Vera Sans", "DejaVu Sans", "Tahoma", "Helvetica",
			"Arial" };
	private static String fontPreferred;
	
	Logger log = LogHandler.getLogInstance();

	// This is the first method that will run the UI after it is constructed.
	// DoITstart will run this.
	...
	protected void setUiLookAndFeel() {
		try {
			...
			laf.getDefaults().put("defaultFont",
					new Font(getPreferredFont(), Font.PLAIN, 12));
			...
			LogHandler.getLogInstance().log(Level.WARNING,
					"Error encountered when setting look and feel", e);
		}
	}
		...
		for (String fontName : fontsPreferred) {
			if (Arrays.binarySearch(fontsInSystem, fontName) >= 0) {
				if (fontPreferred == null) {
					...
					log.info("Font chosen: " + fontPreferred);
				}
				return fontPreferred;
			}
		}
